...
float charToFloatConv(char* str){
  char *remaining;
...
int main (int argc, char* argv[]) {
  ...
  float functionId = charToFloatConv(argv[1]);;
  float a = charToFloatConv(argv[2]);
  float b = charToFloatConv(argv[3]);
  int n = charToFloatConv(argv[4]);
  int intensity = charToFloatConv(argv[5]);
