...
float calculateInt(float functionid, float a, float b, int n, int intensity, int nbthreads, int granularity){
  ...
  sl.parfor<float>(0, n, 1, [&](float& tls) -> void{
  tls = 0;
    ...
    switch ((int) functionid){
      ...
      break;
      default:
        std::cerr<<"FunctionID is not correct!"<<std::endl;
        exit(0);
    }
  }, [&](float &tls) -> void{
...
int main (int argc, char* argv[]) {
  ...
  float response = (float) calculateInt(functionid, a, b, n, intensity, nbthreads, granularity);

  auto stoptime = std::chrono::steady_clock::now();
  std::chrono::duration<double> difference = stoptime - starttime;
